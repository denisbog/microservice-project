...
impl Auth for AuthService {
    ...
    async fn sign_up(
        ...
        match result {
            Ok(_) => {
                ...
                response.set_status_code(StatusCode::Success);
                Ok(Response::new(response))
            }
            Err(_) => {
                let mut response = SignUpResponse::default();
                response.set_status_code(StatusCode::Failure);
                Ok(Response::new(response))
            }
        }
    }
    ...
    async fn sign_out(
        ...
        let mut reply = SignOutResponse::default(); // Create `SignOutResponse` with `status_code` set to `Success`
        reply.set_status_code(StatusCode::Success);
        Ok(Response::new(reply))
    }
}
